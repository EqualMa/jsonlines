...
export function randomBytesSync(size: MaybeRandomNumber): Buffer {
  return crypto.randomBytes(maybeRandomInt(size));
}

export function* iterRandomBytesSync(
  len: number,
  size: MaybeRandomNumber,
): Generator<Buffer> {
  for (let i = 0; i < len; i++) {
...
export async function* iterRandomBytes(
  len: number,
  size: MaybeRandomNumber,
